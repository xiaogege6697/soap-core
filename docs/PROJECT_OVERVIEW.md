...
> 2026-09-14 接续说明：下文保留研究演进，不是当前产品功能清单；当前状态与验证范围见 [历史与能力核对](HISTORY_AND_CAPABILITIES.md)。
...
- **v0.7.3 Multi-layer Representation**：逐层检测（hidden_0/hidden_1）+ 三种聚合。**mode_collapse 首现深层 hidden_1**（eff_rank 1.00 坍缩），浅层正常——验证 collapse 是局部现象。**any_layer 聚合在当前两层 toy benchmark 上 4/4 正确**（normal/overfit 不误判，divergence/mc 检出）；⚠️ 层数增加后多重比较会抬升误报率，多层误报风险待 v0.7.4 验证。consecutive（toy 2层）太严、severity_weighted 漏判 mc（全层均值掩盖局部）。单层 adapter 直接复用。详见 docs/v0.7.3_multilayer_representation_benchmark.md。
- **v0.7.4 HF Multi-layer Benchmark**：预训练 DistilBERT（distilbert-base-uncased 6 层 768）在合成 5 类任务上验证 representation collapse 逐层检测 / 误报率 / 传播路径。**controlled progressive rank-1 activation intervention**（sample pooled space 构造 rank-1 写回 hidden；α 0→0.98 渐进；PCA 第一主方向 u 固定；只 CE loss；**人工干预非自然 collapse**）。**induced collapse 3/3 检出**（首现层 layer_4，传播 layer_4→5→6 三 seed 一致），layer_4 collapse_score 0.974 vs 阈值 0.106（9 倍区分）。**层数增加暴露误报（仅本 controlled benchmark）**：预训练 DistilBERT 合成任务 normal run 深层后期 eff_rank 自然降，深层 per-layer FPR 1/3（3 seed 小样本，v0.7.3 toy 2 层未暴露），persistent 未降误报。冻结层 FPR 0。保留 4 轮失败机制负面结果（effective_rank 归一化，干预须与检测同空间）。**结论限于 controlled benchmark，自然 mode collapse 未验证**。详见 docs/v0.7.4_hf_multilayer_benchmark.md。
- **v0.7.5 Phase-aware Baseline Deviation Detection**：预训练 DistilBERT 合成任务上，phase-knots 基线（8 knots 线性插值 median/MAD + MAD layer floor + run-level max anomaly，threshold_B=10.79 @q0.95）替代 static per-layer threshold。A/B/C 对比（held normal 20 / held induced 3，dev normal 42-53 校准）：A static FA 20/20（Wilson 0.84-1.00），B phase FA 0/20（0.00-0.16），C B+persistent FA 0/20；都 Det 3/3。Exact McNemar A vs B/C n10=20 p=0.000002，B vs C p=1.0。**实验方向选择 B（替代 A），C 暂不采用（无增益）；通用 `soap analyze` 尚未接入此训练诊断，当前使用独立 phase_baseline 模块，统一 CLI 仍为候选计划**。只称 baseline deviation detection，**不宣称**区分良性 NC vs 病态 collapse；controlled benchmark 不外推；phase baseline 须按模型/层/任务重校准。详见 docs/v0.7.5_phase_aware_benchmark.md。
